    ...
    <div class="col-md-8">
        <div class="card shadow">
            ...
            <div class="card-body">
                <table class="table ">
                    ...
                    @foreach ($cart_items as $index => $cart)
                        <tr>
                            <td>{{ ++$index }}</td>
                            <td>
                                <a href="{{ route('customer.product',[$cart->product->id]) }}" title="Go To {{ $cart->product->name }}'s Page'" class="d-flex align-items-start">
                            ...
                            <td>
                                <form action="{{ route('cart.destroy',[$cart->id]) }}" method="post"
                                    ...
                                    <a href="{{ route('cart.edit',[$cart->id]) }}" class="ml-3" title="Edit">
                ...
                <a href="{{ route('cart.confirm') }}" class="btn btn-primary float-right mb-4 mr-2"> Proceed </a>
            </div>
        </div>
    </div>
</div>
